...
- изменение параметров блока (ширина/высота) и обновление кода после нажатия `Generate preview`;
- контроль ошибок в рантайме: тест падает при `pageerror` или `console.error`, а детали прикладываются.

## Структура проекта

Проект построен по облегчённому POM паттерну:
...
- поле с кодом (`textarea#code`) уже заполнено даже до нажатия `Generate preview`;
...
1. Запустите тесты (данные будут собраны в `allure-results/`):
